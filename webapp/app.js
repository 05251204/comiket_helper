...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('hold-counter-container').addEventListener('click', () => {
        if (holdList.length === 0) return;
        if (confirm(`${holdList.length}件の保留リストを完全にリセットしますか？`)) {
            holdList = [];
            localStorage.removeItem('holdList');
            // 操作履歴からも'hold'タイプのものを削除。
            actionHistory = actionHistory.filter(action => action.type !== 'hold');
...
document.getElementById('purchased-btn').addEventListener('click', (event) => {
    ...
    if (webAppURL) {
        ...
        .then(res => res.json())
        .then(data => {
            if (data.status !== 'success') console.error('Background sheet update failed:', data.message);
            else console.log(`Background sheet update successful for ${spaceToUpdate}.`);
        })
        .catch(error => console.error('Background sheet update fetch failed:', error));
    } else {
    ...
    if (lastAction.type === 'purchase') {
        ...
        if (indexToRemove > -1) {
            ...
            if (webAppURL) {
                ...
                .then(res => res.json())
                .then(data => {
                    if (data.status !== 'success') console.error('Undo sheet update failed:', data.message);
                    else console.log(`Undo sheet update successful for ${lastAction.space}.`);
                })
                .catch(error => console.error('Undo sheet update fetch failed:', error));
            }
...
document.getElementById('reset-list-btn').addEventListener('click', (event) => {
    ...
    if (confirm('購入リストを完全にリセットしますか？（スプレッドシートの情報もリセットされます）')) {
        ...
        if (itemsToReset.length > 0) {
            ...
            .then(res => res.json())
            .then(data => {
                if (data.status !== 'success') console.error('Background sheet batch reset failed:', data.message);
                else console.log('Background sheet batch reset successful.');
            })
            .catch(error => console.error('Background sheet batch reset fetch failed:', error));
        }

        // ローカルの購入リストと関連履歴をリセット。
        purchasedList = [];
        localStorage.removeItem('purchasedList');
        actionHistory = actionHistory.filter(action => action.type !== 'purchase');
        localStorage.setItem('actionHistory', JSON.stringify(actionHistory));
...
function updateNextTarget() {
    ...
    const remainingCircles = comiketData.wantToBuy.filter(c => 
        !purchasedList.includes(c.space) && !holdList.includes(c.space)
    );
    ...
    setTimeout(() => {
        ...
        document.getElementById('target-space-heading').textContent = nextCircle.space;
        ...
        if (nextCircle.tweet) {
            ...
            if (typeof twttr !== 'undefined' && twttr.widgets) {
                ...
                if (tweetIdMatch && tweetIdMatch[1]) {
                    ...
                    twttr.widgets.createTweet(tweetIdMatch[1], embedContainer, { theme: 'light' })
                        .then(el => {
                            if (!el) {
                                // Widget creation failed (e.g., for sensitive content), the link is already there.
                                console.warn('Tweet widget creation failed, possibly sensitive content.');
                            }
                        })
                        .catch(err => { 
...
function updateAllCounts() {
    const unvisitedCircles = comiketData.wantToBuy.filter(c => 
        !purchasedList.includes(c.space) && !holdList.includes(c.space)
    );
